...
struct TestRowNullable<'a> {
    pub Id: i32,
    pub VarCharRow: Option<&'a str>,
    pub NVarCharRow: Option<&'a str>,
    ...
    pub FloatRow: Option<f32>, // borked. Breaks because tiberius inteprets a a nullable float field as F32(None)
...
#[tokio::test]
async fn by_ref_nullable() -> Result<(), tiberius::error::Error> {
    let mut client = connect_localhost().await.unwrap();
    let query = r"
    SELECT
    ...
    let expected_1 = TestRowNullable {
        Id: 1,
        VarCharRow: "varchar".into(),
        NVarCharRow: "nvarchar".into(),
        LongRow: 9999999999999999.into(),
        SmallIntRow: 2.into(),
        BitRow: true.into(),
        FloatRow: 10.123123125.into(),
        //  DoubleRow: 99.1231231258.into(),
        RealRow: 10.5.into(),
